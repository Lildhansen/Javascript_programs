...
function calcIdentical(diceCount,N,numToStartFrom=6){
  for(let i=d2i(numToStartFrom);i>=d2i(minDice);i--)
   if(diceCount[i]>=N) 
      return i2d(i)*N;
  return 0;
}
...
function calcTwoPairs(diceCount){
  let pair1 = calcIdentical(diceCount,2);
  let pair2 = calcIdentical(diceCount,2,(pair1/2)-1);
  if (pair1 && pair2)
    return pair1 + pair2;
  return 0;
}
...
function printScoresHTML(play){
  ...
  res+="<p>Choose your yatzy game</p>"
  res+="<select name='yatzyGames' id='yatzyGames'> <option value='game1'>Yatzy</option> <option value='game2'>Super Yatzy</option> <option value='game3'>Mega Yatzy</option> <option value='game4'>Ultra Yatzy</option> </select>"
  ...
  for(let round=0;round<play.length;round++) {
    ...
    if(!isSpecialRound(round)){
      ...
      for(let d=0;d<play[round].diceRoll.length;d++){
         res+=`<img src="resources/${play[round].diceRoll[d]}-dice.png" width="20" height="20" alt="picture of die with number: ${play[round].diceRoll[d]}}">`;
      }
      res+="</span>";
    }
    res+="</td>"
  ...
  res+="</tbody></table>";
  res+="<p>Check rules <a href='help.html'>here</a></p>"
