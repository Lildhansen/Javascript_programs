...
let questions = [new Question("what is life?","no","yes","maybe",3),new Question("what is red?","red","blue","yellow",1)];
...
server.on("request", (request, response) => {
    ...
    response.writeHead(404).end();
        ...
        promptQuestion(request,response)
        setInterval(promptQuestion(request,res), 1000*60);
    }
    console.log(request.method);
});

function promptQuestion(request,response)
{
    let randomQuestion = questions[Math.floor(Math.random() * questions.length)];
    console.log(randomQuestion.isCorrectAnswer());
