...
function yatzyHighScorePage(validatedData){
  //Casper's nice shit
  let rows = "";
  if ( yatzyController.games.length === 0 ) {
    rows = "<tr><td> no history </tr></td>"
  }
  else {
    for (let i = 0; i < yatzyController.games.length; i++) {
      let currentgame = yatzyController.games[i];
      rows += "<tr>"
      rows += `<th>game number  ${i + 1}</th> `
      rows += `<td> ${currentgame.name}</td> `
      rows += `<td> ${currentgame.scoreTable[17].score}</td> `
      rows += "</tr>"
    }
  }

  let str = ` ${printHTMLHdr("Highscores", ["css/style.css"])}
  <body>
  <table id="scoretable">
  ${rows}
  </table>
  ...
  let testHighScore = [69420,420,69,5,-7000000];
  let firstPart = `<fieldset>;
  <legend>Highscores</legend> </fieldset>\n`;
  firstPart += "<table>"
  for (let i = 0; i < testHighScore.length;i)
  {
    firstPart += "<tr>" + "<td>" + testHighScore[i] + "</td>" + "<tr>";
  }
  firstPart += "</table>"
  let page = ""
  page += printHTMLHdr("Highscores",["css/style.css"]);
  page += printHTMLBody(firstPart);
  return page;*/
}
...
function YatzyGame(gameID,name,diceCount){
  ...
  this.printGameFormHTML= function (){
    ...
    <a href="/html/help.html"' ${(this.gameOn && !this.gameOver()) ? "" : "disabled"}> Rules</a>
  ...
  <a href='http://127.0.0.1:3000/'>Tilbage</a>
  <a href="/html/help.html"' ${(this.gameOn && !this.gameOver()) ? "" : "disabled"}> Rules</a>
  ...
  <form id="gameControl_id"  action="http://127.0.0.1:3000/highscores" method="get">
  <input type="submit" value="Highscore" ${(this.gameOn && this.gameOver()) ? "" : "disabled"}></intput>
  </form>`;
...
/*<form id="gameControl_id"  action="http://127.0.0.1:3000/highscores" method="GET">
  <input type="submit" value="High Scores" ${(this.gameOn && this.gameOver()) ? "" : "disabled"}>
  </form>*/
}
